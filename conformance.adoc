...
* An aggregation variable has an **`aggregated_dimensions`** attribute whose string value is a blank-separated list of zero or more aggregated dimension names.

* Each aggregated dimension must name a dimension in the file.

* An aggregation variable must be a scalar.
...
* A `uris` variable must have a string data type.

  ** The `uris` variable must have the same number of dimensions as there are aggregated dimensions.
  ** The size of each dimension must be the equal to number of valid values in the corresponding row of the `map` variable.
  ** The variable must have no missing values.
  ** A data value of a `uris` variable must be either an absolute URI or else a relative-path URI reference.

* An `identifiers` variable must be either a scalar, or else have the same dimensions in the same order as the `uris` variable.
  ** The variable must have no missing values.

* The `unique_values` variable must have the same number of dimensions as there are aggregated dimensions.
  ** The size of each dimension must be equal to the number of valid values in the corresponding row of the `map` variable.
  
* The `map` variable must have an integer data type.
  ** If there are zero aggregated dimensions then the `map` variable must a be scalar and contain the value `1`.
  ** If there are one or more aggregated dimensions then the `map` variable must be two-dimensional.
  *** The size of the slower-varying dimension (i.e. the first dimension in CDL order, representing rows) must be the number of aggregated dimensions.
  *** The sum of the valid values in a row must equal the size of the corresponding aggregated dimension.

*Recommendations:*

* The following kinds of variable should not be aggregation variables: grid mapping variables, domain variables, mesh topology variables, geometry container variables, interpolation variables, and quantization variables.
...
In terms of the <<UDUNITS-grammar>>, CF does not allow **`INT`** or **`REAL`** in a string containing **`ID`**, except for the specification of a reference time in the units of time coordinates.
...
* A non-zero time zone offset must not appear in the reference datetime in the **`utc`** and **`tai`** calendars.
* The reference datetime must be written in the correct format.
In particular, it must include the date, and the time zone offset must not be included unless the time is included.

*Recommendations:*

In the first three recommendations, when we refer to a UDUNITS unit of time e.g. **`year`**, we mean the string given or any which is equivalent to it in UDUNITS e.g. **`yr`**.

* Units of **`year`** and **`month`** should not be used in any calendar.
* Units of **`day`**, **`hour`** and **`minute`** should not be used in the **`utc`** calendar. In that calendar, the only recommended unit is **`second`**.
* The decimal prefixes of Table 3.1 should not be used with any unit of time apart from **`second`**.
...
* A non-zero time zone offset should not be specified in the reference datetime.
...
* Datetimes before 1972-01-01 0:0:0 and after the current instant (i.e. any time in the future when the data is written) are invalid in the **`utc`** calendar, datetimes before 1958-01-01 0:0:0 are invalid in the **`tai`** calendar, datetimes in negative years are invalid in the **`standard`** and **`julian`** calendars, and datetimes in the range from (and including) 1582-10-5 0:0:0 until (but excluding) 1582-10-15 0:0:0 are invalid in the **`standard`** calendar.
Datetimes which are invalid in the specified calendar must not be encoded as time coordinates.
* The reference datetime of a time coordinate variable must be a valid datetime in the specified calendar.
...
* In the **`standard`** calendar, a reference datetime before 1582-10-5 0:0:0 should not be used for a time coordinate representing 1582-10-15 0:0:0 or a later datetime, and a reference datetime of 1582-10-15 0:0:0 or later should not be used for a time coordinate representing a datetime earlier than 1582-10-5 0:0:0.
* A datetime during a leap second (i.e. with a value for seconds that equals or exceeds 60) should not be used as the reference datetime of a time coordinate variable if the **`calendar`** is **`utc`**.
(In other calendars, leap seconds must not be used because they are not valid datetimes.)

[[explicitly-defined-calendar]]
=== 4.4.4 Explicitly Defined Calendar
...
* The data types of the attributes of the grid mapping variable must be as specified in Table F.1 of Appendix F. +
...
* If the value of the **`indexing_scheme`** attribute is **`nested`** or **`ring`**, then the **`refinement_level`** attribute must also be present.
...
* An auxiliary coordinate variable with standard name **`healpix_index`**, of size greater than one, all of whose values are distinct, should be a coordinate variable instead (with values ordered monotonically).
...
* If the associated coordinate variable is one-dimensional and of size greater than one, the bounds of each cell must be ordered in the same sense as the coordinates (increasing or decreasing).
...
* In a **`cell_methods`** entry of the form __name__**`:`** [__name__**`:`** ...] **`anomaly_wrt`** __norm__, the word __norm__ must be the name of a variable that is present in the file, and is named by the **`ancillary_variables`** attribute of the data variable (hence we hereafter refer to it as the "norm ancillary variable").
* Each of the coordinate variables and scalar coordinate variables identified by the __name__s in the **`anomaly_wrt`** entry of **`cell_methods`** (we refer to these variables hereafter as "anomaly coordinate variables") must have a **`standard_name`** attribute.
* If either the data variable or the norm ancillary variable has a **`standard_name`** attribute, its value must not end in **`_anomaly`**, and if they both have **`standard_name`** attributes, they must have the same value.
* The anomaly coordinate variables must not be coordinate variables or scalar coordinate variables of the norm ancillary variable.
* The coordinate variables and scalar coordinate variables of the norm ancillary variable must satisfy one of the following conditions:
1. The norm ancillary variable has all the coordinate variables (and their dimensions) and the scalar coordinate variables of the data variable, except for the anomaly coordinate variables.
2. The norm ancillary variable has none of the coordinate variables or scalar coordinate variables of the data variable.
3. The norm ancillary variable has no scalar coordinate variables, and its sole dimension is the dimension of an anomaly coordinate variable of time.
* In cases (1) and (2), for each of the anomaly coordinate variables, the norm ancillary variable must have either a coordinate variable with a size-one dimension, or a scalar coordinate variable, with the same **`standard_name`** attribute as the anomaly coordinate variable, and each of these variables must have either a **`bounds`** or a **`climatology`** attribute. (We hereafter refer to these variables as "norm coordinate variables.)
* In cases (1) and (2), the norm ancillary variable must have a **`cell_methods`** attribute with an entry that refers to all the norm coordinate variables, or more than one entry if they contain **`within`** and **`over`** keywords.
* In case (3), the norm ancillary variable must have a **`cell_methods`** attribute comprising entries containing **`within`** and **`over`** keywords and referring to its sole dimension.
* In cases (2) and (3), **`cell_methods**` must have no entries other that the ones described in the previous two points.
* In case (3), the norm ancillary variable must have an auxiliary coordinate variable of integer type with a **`select`** attribute naming a time coordinate variable that has a **`climatology`** attribute. The auxiliary coordinate values must be non-negative and less than the dimension named by the **`select`** attribute.
