...
The date/time-set of the **`utc`** calendar includes leap seconds, where by "date/time-set" we mean the complete set of valid year-month-day-hour-minute-second combinations.
The date/time-sets of all _other_ calendars, _except_ **`standard`**, **`proleptic_gregorian`**, and **`julian`**, do _not_ include leap seconds.
The **`standard`**, **`proleptic_gregorian`**, and **`julian`** calendars are special in that they can be used for date/time-sets either __with__ or __without__ leap seconds, 

For these calendars, there are the following cases:

1. **The calendar is being used for a date/time-set in which leap seconds do not exist.**
   This is the case for a model simulation that defines every day as having a constant length of 86400 seconds.

2. **The calendar is being used for a date/time-set in which leap seconds exist, and are correctly accounted for.**
...
3. **The calendar is being used for a date/time-set in which leap seconds exist, but some or all leap seconds might not have not been correctly accounted for.**
...
4. **It may be unknown whether leap seconds exist in the date/time-set.**
...
When the these calendars are being be used for date/time-sets __with__ leap seconds (i.e. cases 2 and 3 and perhaps case 4), this assumption has the following consequences:
...
* If any leap seconds occurred between the reference date/time and the date/time represented by a time coordinate, the coordinate value does __not__ count the leap seconds (assuming that the leap seconds were known when the time coordinate was calculated).
  ...
  By contrast, in case 1 (i.e. a date/time-set without leap seconds), the difference between two time coordinates is always equal to the length of time between the instants they represent, as is always true in all other calendars.
  It is important to be aware of this disadvantage of the **`standard`**, **`proleptic_gregorian`** and **`julian`** calendars when used with date/time-sets including leap seconds.
...
**`none`** means that leap seconds do not exist in the date/time-set (i.e. case 1), **`utc`** means that leap seconds exist in the date/time-set and are correctly accounted for (i.e. case 2), and **`unknown`** means that the data-writer did not know or did not record whether the leap seconds exist in the date/time-set, nor how they are treated if they did exist (i.e. cases 3 and 4).
...
data:
  ...
  time2 = 1; // 2017-1-1 0:0:0 because no leap seconds in date/time-set
  time3 = 1; // 2017-1-1 0:0:0 because leap second is not counted
  time4 = 1; // leap second 2016-12-31 23:59:60
  time5 = 1; // 2017-1-1 0:0:0 because no leap seconds in date/time-set
----
====
