...
Plural forms are also acceptable.
In CF, following UDUNITS, any unit may optionally have one of the decimal prefixes for multiples and submultiples (<<table-supported-units, Table 3.1>>) e.g. **`millisecond`** or **`ms`**; however, we note that <<SI>> (chapter 4) does not allow these prefixes for any unit of time other than **`second`**.
...
[options="header",caption="Table 4.1. ",cols="18,12,10,18,10,10"]
|===============
| **`calendar`** | Days in year | Leap days | Leap seconds | First date | Last date
| **`standard`** | 365/366 | Julian/
Gregorian | before CF 1.13: unknown
since CF 1.13: no | 0001-01-01 | +&#x221E;
| **`julian`** | 365/366 | Julian | no | 0001-01-01 | +&#x221E;
| **`proleptic_gregorian`** | 365/366 | Gregorian | no | -&#x221E; | +&#x221E;
| **`no_leap`**/**`365_day`** | 365 | never | no | -&#x221E; | +&#x221E;
| **`all_leap`**/**`366_day`** | 366 | every year | no | -&#x221E; | +&#x221E;
| **`360_day`** | 360 | never | no | -&#x221E; | +&#x221E;
| **`utc`** | 365/366 | Gregorian | yes | 1972-01-01 | present day
| **`tai`** | 365/366 | Gregorian | no | 1958-01-01 | +&#x221E;
|===============
...
##In the **Leap seconds** column, "no" means that seconds equal to or exceeding 60 are not allowed in datetimes, and that time coordinates and datetimes must be interconverted assuming 60 seconds in every minute, as if leap seconds never occurred.##
====
...
Leap seconds do not need to be considered for model-generated data in any model calendar.
Leap seconds should be considered:

* if you are using an existing observational dataset for dates since 1972 with time coordinates in the **`standard`**pass:q[<del><span style="color: red">, `proleptic_gregorian`</span></del>] or **`utc`** calendar, which treat them differently.
Later paragraphs of this section give further information about these cases.

* if you are using an existing observational dataset with time coordinates in the **`tai`** calendar.
These do not give UTC when converted to datetimes.
See the later paragraph about the **`tai`** calendar.

* if you are writing an observational dataset for dates since 1972 using datetimes from a clock or data logging device whose time and date are related to civil time e.g. clocks based on UTC or computers obtaining time signals from an NTP server.
pass:q[<del><span style="color: red">It is recommended to use the `proleptic_gregorian` or the `utc` calendar in preference to the `standard` calendar in this case.</span></del>]
##In order to preserve precision to the second in all datetimes and time intervals, you need to use the **`utc`** calendar instead of the **`standard`** calendar.##
<<appendix-leap-seconds>> compares these calendars' treatment of leap seconds in detail.
pass:q[<del><span style="color: red">You need the `utc` calendar to preserve precision to the second in all datetimes and differences between time coordinates.</span></del>]
...
* if you are writing a model dataset following the real-world Gregorian calendar for dates since 1972.
It is recommended to use the **`proleptic_gregorian`** calendar in preference to the **`standard`** calendar in this case, in order to be completely clear to the dataset user that there are no leap seconds.
...
For pass:q[<del><span style="color: red">existing</span></del>] ##such## datasets, this uncertainty can be eliminated only if you have separate information about how leap seconds were treated.
+
##In CF 1.13 or later, it is recommended to write data from models that follow the Gregorian rule for leap days in the **`proleptic_gregorian`** calendar rather than the **`standard`** calendar.
...
**`proleptic_gregorian`**:: A ##model## calendar with the Gregorian rule for leap years, extended to dates before 1582-10-15.
All dates consistent with these rules are allowed, both before and after 1582-10-15 0:0:0.
##There are no leap seconds in this calendar.##
pass:q[<del><span style="color: red">If the time coordinates identify real-world instants from 1972 onwards, they represent UTC datetimes.
In this case, the difference between two time coordinates will not exactly equal the length of the interval between the instants concerned if there are any intervening leap seconds.
See `appendix-leap-seconds` for details.</span></del>]
...
If time coordinates in the **`utc`** calendar are converted to datetimes by software that is unaware of leap seconds, they will be later than the correct UTC datetimes by a number of seconds.
+
